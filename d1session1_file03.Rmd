...
```{r}
# save mean diameter of abalones
meandiam <- abalone %>%
  pull(diameter) %>%
  mean()
meandiam
...
The mean diameter of all of the abalones is `r meandiam`.
...
abalone %>%
  group_by(sex) %>%
  summarise(median = median(wholeWeight),
            iqr = quantile(wholeWeight, c(0.25, 0.75)))
